...
import {
  TestTube, Upload, Eye, ShieldCheck, Download, Printer, XCircle, Search,
  Clock, Activity, CheckCircle, AlertCircle, FlaskConical, Microscope,
...
export default function LabTestList({ title, description, statusFilter, showActions = true }) {
  ...
  const processing = rows.filter((t) => t.status === 'sample_collected' || t.status === 'in_progress');
  const completed = rows.filter((t) => t.status === 'completed');
  const verified = rows.filter((t) => t.status === 'verified');
  ...
  return (
    <div>
      <PageHeader title={title} description={description} breadcrumb="Laboratory" />

      {/* Stats */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <StatCard icon={Clock} label="Pending" value={pending.length} color="warning" delay={0} />
        <StatCard icon={Activity} label="Processing" value={processing.length} color="primary" delay={0.05} />
        <StatCard icon={CheckCircle} label="Completed" value={completed.length} color="accent" delay={0.1} />
        <StatCard icon={Eye} label="Verified" value={verified.length} color="secondary" delay={0.15} />
        <StatCard icon={ShieldCheck} label="Approved" value={approved.length} color="success" delay={0.2} />
      </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-neutral-100 bg-neutral-50/50">
                ...
                {showActions && <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-neutral-500">Actions</th>}
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-100">
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i}>{Array.from({ length: showActions ? 6 : 5 }).map((_, j) => <td key={j} className="px-5 py-4"><div className="skeleton h-4 w-full max-w-[120px]" /></td>)}</tr>
                ))
              ) : filtered.length === 0 ? (
                <tr><td colSpan={showActions ? 6 : 5}><EmptyState icon={FlaskConical} title="No tests found" description="Tests will appear here as they move through the workflow." /></td></tr>
              ) : (
                filtered.map((row, i) => (
      ...
      <Modal open={!!rejectTarget} onClose={() => setRejectTarget(null)} title={`Reject Report — ${rejectTarget?.testName || ''}`}>
        <div className="space-y-4">
          <div className="rounded-xl bg-error-50/50 border border-error-100 p-3.5">
            <p className="text-sm text-error-700">Rejection sends the test back to the lab team.</p>
          </div>
          <div><label className="label">Reason</label><textarea rows={3} required value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} className="input" placeholder="e.g. Sample hemolyzed." /></div>
